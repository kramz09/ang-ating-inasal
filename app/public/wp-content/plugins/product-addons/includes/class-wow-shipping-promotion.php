 ...
 * @package PRAD
 */
namespace PRAD\Includes; // CHANGE THIS.
...
class WowShippingPromotion {
	...
	private const MENU_SLUG            = 'prad-dashboard'; // CHANGE THIS.
	...
	public function add_submenu() {
		...
		$url = admin_url( 'admin.php?page=prad-dashboard#our_product/wow_shipping' ); // CHANGE THIS.
		...
		add_submenu_page(
			...
			'Add Shipping Rules',
			$submenu_content,
			$menupage_cap, // CHANGE THIS IF NEEDED.
			'wtrs-promotion',
			'__return_false'
		);
	}
	...
	public function ajax_install_promotion_plugin() {
		...
		if ( ! $plugin_exists && ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'You are not allowed to install plugins.', 'product-addons' ),
				)
			);
		}
		...
		if ( $plugin_exists && ! current_user_can( 'activate_plugins' ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'You are not allowed to activate plugins.', 'product-addons' ),
				)
			);
		}
		...
		if ( false === $result ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Failed to install WowShipping.', 'product-addons' ),
				)
			);
		}
		...
		wp_send_json_success(
			array(
				...
				'message'       => esc_html__( 'WowShipping installed successfully.', 'product-addons' ),
			)
		);
	}
	...
	/**
	 * Insert a promotion field after the Product dimension unit setting.
	 *
	...
	private function render_promotion_notice( $id, $type, $message, $style = '', $inline = true, $button_labels = array() ) {
		...
		>
			...
			<button type="button" class="notice-dismiss">
				<span class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'product-addons' ); ?></span>
			</button>
		</div>
		...
		<script>
			jQuery( function( $ ) {
				$( document ).on( 'click', '<?php echo esc_js( $id ); ?> .wtrs-promotion-install-link', function( event ) {
					...
					var defaultErrorMessage = <?php echo wp_json_encode( esc_html__( 'Failed to install WowShipping.', 'product-addons' ) ); ?>;
