    ...
    public static function get_inline_style_scheme($style_schemes, $slug, $form_id = array(), $form_excluded = array(), $form_prefix = '') {
        ...
        if (!empty($form_excluded)) {
            foreach ($form_excluded as $form_excluded_id => $form_excluded_style) {
                ...
                $form = get_post($form_excluded_id);
                if ($form && get_post_type( $form ) === 'wpcf7_contact_form') {
                    $hash = substr(get_post_meta( $form_excluded_id, '_hash', true ), 0, 7 );
                    if (!empty($hash)) $form_excluded_id = $hash;
                    $wrappers[] = $form_prefix . '#cf7cstmzr-form:not(.cf7cstmzr-form-'.$form_excluded_id.') ';
                }
            ...
            foreach ($form_id as $item) {
                ...
                $form = get_post($item);
                if ($form && get_post_type( $form ) === 'wpcf7_contact_form') {
                    $hash = substr(get_post_meta( $item, '_hash', true ), 0, 7 );
                    if (!empty($hash)) $item = $hash;
                    $wrappers[] = $form_prefix . '#cf7cstmzr-form.cf7cstmzr-form-'.$item.' ';
                }
            }
        } else {
 ...
 */
$selectors = array( '.wpcf7-form:before' );
$selectors_wrapper = array();
...
<?php if (!empty($style_scheme["checkbox_full_width"])) { ?>
/* Style Form Checkboxes Width */
<?php self::style_selectors(array('.wpcf7-form .wpcf7-checkbox > span'), $wrappers) ?> {
    <?php
        if ('yes' === $style_scheme["checkbox_full_width"]) {
            echo PHP_EOL . "\t" . "display: block;";
            echo PHP_EOL . "\t" . "width: 100%;";
        } elseif ('no' === $style_scheme["checkbox_full_width"]) {
...
<?php } ?>
    ...
    <?php if (!empty($style_scheme["radiobutton_full_width"])) { ?>
/* Style Form Radiobuttons Width */
<?php self::style_selectors(array('.wpcf7-form .wpcf7-radio > span'), $wrappers) ?> {
    <?php
        if ('yes' === $style_scheme["radiobutton_full_width"]) {
            echo PHP_EOL . "\t" . "display: block;";
            echo PHP_EOL . "\t" . "width: 100%;";
        } elseif ('no' === $style_scheme["radiobutton_full_width"]) {
            echo PHP_EOL . "\t" . "display: inline-block;";
            echo PHP_EOL . "\t" . "width: auto;";
        }
    ?>
...
<?php if (!empty($style_scheme["checkbox_text_label_size"])) { ?>
<?php
$selectors = array(
...
<?php } ?>
    ...
    <?php
    if (
        !empty($style_scheme["form_text_label_color"]) ||
        !empty($style_scheme["form_text_color"]) ||
        !empty($style_scheme["form_text_label_size"]) ||
        !empty($style_scheme['form_text_label_weight']) ||
        !empty($style_scheme['form_text_label_style'])
    ) { ?>
/* Style Form Labels */
