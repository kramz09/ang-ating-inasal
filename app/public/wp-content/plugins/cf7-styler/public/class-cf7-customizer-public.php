...
class Cf7_Customizer_Public {
	...
	public function add_wrapper($output, $tag, $attr, $m) {
	    if ('contact-form-7' === sanitize_text_field($tag)) {
            ...
            if (empty($is_form_exists) && !empty($attr["id"])) {
                if (function_exists('wpcf7_get_contact_form_by_hash')) {
                    $is_form_exists = wpcf7_get_contact_form_by_hash( $attr["id"] );
                }

                if ( ! $is_form_exists && function_exists('wpcf7_contact_form') ) {
                    $is_form_exists = wpcf7_contact_form( $attr["id"] );
                }
            }
